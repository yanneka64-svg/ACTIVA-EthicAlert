...
| 4 | Control Panel (Overview, Alerts, Triage, Assignment, SLA & Escalations views) | 🛑 Blocked indefinitely on Spark (same root cause as Phase 3) | Firestore rejects `list`/query operations outright (not a silent per-document filter) whenever the rule depends on a field no query `where` clause constrains — here, `implicatedUserIds` (rule #9). Verified live: even `functional_admin` gets `403` trying to list `cases`. Single-document `getDoc` of one known id still works correctly (extended to subcollections too, deployed). A real "list of cases" UI needs a Cloud Function, and Cloud Functions won't deploy on Spark (see Phase 3) — so this stays blocked for as long as the Spark decision stands, not just "for now". `src/services/firebaseClient.ts` (real Firebase Web App registered + connected, config in `.env`) is ready for whenever that changes. See `docs/FIREBASE-SETUP.md` for the two honest options if a list-capable screen is wanted without Functions. |
