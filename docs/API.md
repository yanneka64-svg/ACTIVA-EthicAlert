...
| `addPerson` | Request `{ caseId, kind, name, ... }` (the rest of `Person`, minus server-set fields) → `{ personId }`. `kind` is validated to be exactly `'subject'` or `'witness'`. **=== AMÉLIORATION AJOUTÉE : la vérification de sécurité la plus importante de ce fichier ===** when `kind === 'subject'` and `linkedUserId` is set, this appends to `Case.implicatedUserIds` (`FieldValue.arrayUnion`) in the same call — the one write in the entire file that `firestore.rules`' `isNotImplicated` check directly depends on (rule #9, `docs/PERMISSIONS.md`). Skipping or reordering this relative to the person-document write would silently reopen the exact gap rule #9 exists to close, so it is called out explicitly in the code, not just here. **A security review found `cases.edit` alone (held by any plain `investigator`) was enough to trigger this — see the fix in "Security review findings & fixes" below: setting `linkedUserId` now additionally requires `cases.assign`, and every link is audited (`PERSON_LINKED_TO_USER`).** |
| `removePersonLink` | Request `{ caseId, personId, reason }` → `{ ok: true }`. Permission required: `cases.assign` — same privilege as setting the link in the first place. Clears `Person.linkedUserId` and removes the uid from `Case.implicatedUserIds` (`FieldValue.arrayRemove`); writes a `PERSON_UNLINKED_FROM_USER` audit entry (with `reason`) and a `PERSON_UNLINKED` timeline event. Added alongside the `addPerson` fix below — until this existed, an implication set via `addPerson` had no reversal path through the API at all. |
