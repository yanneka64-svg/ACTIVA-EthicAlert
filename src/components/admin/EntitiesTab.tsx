...
export const EntitiesTab: React.FC<EntitiesTabProps> = ({ entities, countries, activeUser, onSaved, slugify }) => {
  // --- Entities CRUD state ---
  ...
  const handleSaveEntity = (e: React.FormEvent) => {
    ...
    if (editingEntityId) {
      ...
      onSaved(`Entité "${entityName.trim()}" mise à jour.`);
    } else {
      ...
      onSaved(`Entité "${entityName.trim()}" ajoutée.`);
    }
  ...
  const handleDeleteEntity = (ent: EntityDef) => {
    ...
    onSaved(`Entité "${ent.name}" supprimée.`);
  };
