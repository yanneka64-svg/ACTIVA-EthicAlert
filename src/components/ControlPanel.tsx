    ...
    <div className="max-w-[1500px] mx-auto py-6 px-4 sm:px-6 lg:px-8 space-y-5">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900">{t.cp_title}</h2>
            <p className="text-xs text-slate-600 mt-0.5">{t.cp_subtitle}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-xl p-1 overflow-x-auto">
            {periodOptions.map((opt) => (
              <button
                ...
                className={`px-2.5 py-1.5 rounded-lg text-[11px] font-bold whitespace-nowrap transition flex items-center gap-1 ${
                  period === opt.key ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-50'
                }`}
              >
          ...
          {period === 'custom' && (
            <div className="flex items-center gap-1.5 text-[11px]">
              <input type="date" value={customStart} onChange={(e) => setCustomStart(e.target.value)} className="border border-slate-200 rounded-lg px-2 py-1" />
              <span className="text-slate-400">→</span>
              <input type="date" value={customEnd} onChange={(e) => setCustomEnd(e.target.value)} className="border border-slate-200 rounded-lg px-2 py-1" />
            </div>
          )}
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-slate-200 bg-white text-[11px] font-bold text-slate-600 hover:bg-slate-50 transition"
            >
              <RefreshCw className="w-3.5 h-3.5" /> {t.cp_refresh}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button onClick={onNavigateToNewCase} className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-blue-700 text-white text-xs font-bold hover:bg-blue-800 transition shadow-sm">
          <Plus className="w-4 h-4" /> {t.cp_action_new_case}
        </button>
        <button onClick={() => onNavigateToCases({ status: 'new' })} className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-700 hover:bg-slate-50 transition">
          <Inbox className="w-4 h-4 text-blue-600" /> {t.cp_qa_review_new}
        </button>
        <button onClick={() => onNavigateToCases({ unassignedOnly: true })} className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-700 hover:bg-slate-50 transition">
          <ListChecks className="w-4 h-4 text-amber-600" /> {t.cp_action_triage}
        </button>
        <button onClick={() => onNavigateToCases({ overdueOnly: true })} className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-700 hover:bg-slate-50 transition">
