...
// === AMÉLIORATION AJOUTÉE (rapports PDF réels avec en-tête ACTIVA) ===
// Les 3 modèles proposés à l'utilisateur, partagés par les 2 emplacements
// du bouton "Générer le rapport" (Synthèse du dossier / Liens rapides) —
// voir CaseReportPrintView.tsx pour le contenu réel de chaque modèle.
const REPORT_MODEL_OPTIONS: { key: ReportModel; label: string }[] = [
  { key: 'summary', label: 'Synthèse (1 page)' },
  { key: 'full', label: 'Dossier complet' },
  { key: 'audit', label: 'Audit / conformité (anonymisé)' },
];
...
export const InvestigationDesk: React.FC<InvestigationDeskProps> = ({
  ...
  // === AMÉLIORATION AJOUTÉE (rapports PDF réels avec en-tête ACTIVA) ===
  // `openReportMenu` distingue les 2 emplacements du bouton "Générer le
  // rapport" (Synthèse du dossier / Liens rapides) — un seul menu ouvert à
  // la fois. `printReportType` déclenche l'impression réelle : dès qu'il
  // est renseigné, CaseReportPrintView.tsx (rendu plus bas, cf. `.print-only`
  // dans index.css) devient le SEUL contenu visible dans la boîte de
  // dialogue d'impression du navigateur — jamais toute la page comme
  // auparavant (BUG PRÉEXISTANT CORRIGÉ, signalé par l'utilisateur).
  const [openReportMenu, setOpenReportMenu] = useState<'summary_tab' | 'quick_links' | null>(null);
  ...
  // === AMÉLIORATION AJOUTÉE (rapports PDF réels avec en-tête ACTIVA) ===
  // Ferme le menu de modèles au clic en dehors, comme les autres
  // menus/popovers de l'application.
  useEffect(() => {
    if (!openReportMenu) return;
    const handleClickOutside = (e: MouseEvent) => {
      const ref = openReportMenu === 'summary_tab' ? reportMenuSummaryRef : reportMenuLinksRef;
              ...
              <div className="p-6 space-y-6 max-h-[640px] overflow-y-auto text-xs">
                ...
                <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
                  ...
                  {/* === AMÉLIORATION AJOUTÉE (rapports PDF réels avec
                      ...
                      latérale, navigation comprises) par un choix de
                      modèle, chacun ne déclenchant l'impression que du
                      contenu réel de CaseReportPrintView.tsx. */}
                  <div className="relative inline-block" ref={reportMenuSummaryRef}>
                    <button
                      onClick={() => setOpenReportMenu(openReportMenu === 'summary_tab' ? null : 'summary_tab')}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-bold transition"
                    >
                    ...
                    {openReportMenu === 'summary_tab' && (
                      <div className="absolute left-0 top-full mt-1 z-20 bg-white border border-slate-200 rounded-xl shadow-lg py-1 min-w-[220px]">
                        {REPORT_MODEL_OPTIONS.map((opt) => (
                          <button
                            key={opt.key}
                            onClick={() => handleGenerateReport(opt.key)}
                            className="w-full text-left px-3 py-2 hover:bg-slate-50 text-xs font-semibold text-slate-700"
                          >
                            {opt.label}
                          </button>
                        ))}
          ...
          <aside className="space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-2">
              ...
              <div className="relative" ref={reportMenuLinksRef}>
                <button
                  onClick={() => setOpenReportMenu(openReportMenu === 'quick_links' ? null : 'quick_links')}
