/**
 * === AMÉLIORATION AJOUTÉE (Brancher le vrai backend — Phase 13) ===
...
import { mirrorStaffCaseCreationToRealBackend } from './caseCreationMirrorSync';
...
describe('mirrorStaffCaseCreationToRealBackend', () => {
  it('ne fait rien (no-op silencieux) si Firebase Phase 4 n\'est pas configuré', async () => {
    ...
    const result = await mirrorStaffCaseCreationToRealBackend(input);

    expect(result).toBeNull();
    expect(mockGetPhase4Functions).not.toHaveBeenCalled();
  });

  it('appelle createCase (jamais createCaseAsReporter) et renvoie caseId/caseNumber en cas de succès', async () => {
  ...
  it('renvoie null sans jamais rejeter si l\'appel réseau échoue', async () => {
    ...
    await expect(mirrorStaffCaseCreationToRealBackend(input)).resolves.toBeNull();
  });

  it('renvoie null sans jamais rejeter si le serveur refuse (permission-denied — pas de session réelle)', async () => {
    mockIsPhase4Configured.mockReturnValue(true);
    mockGetPhase4Functions.mockResolvedValue({});
